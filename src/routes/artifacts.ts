...
import { getArtifactsDirectory, readManifest, updateManifest } from '../lib/store.js';
...
function isBucketUpdate(body: unknown): body is { bucket: 'operational' | 'understanding' } {
  return Boolean(body && typeof body === 'object' &&
    ((body as Record<string, unknown>).bucket === 'operational' || (body as Record<string, unknown>).bucket === 'understanding'));
}

function isCreateArtifactBody(body: unknown): body is CreateArtifactBody {
  if (!body || typeof body !== 'object') return false;
  const value = body as Record<string, unknown>;
  return typeof value.html === 'string' && typeof value.title === 'string' &&
...
export async function artifactRoutes(app: FastifyInstance) {
  app.get('/artifacts/:id.html', async (request, reply) => {
    ...
    if (!idPattern.test(id)) return reply.code(404).send({ error: 'not found' });

    const artifact = (await readManifest()).artifacts.find((item) => item.id === id);
    if (!artifact) return reply.code(404).send({ error: 'not found' });

    const html = await readFile(join(getArtifactsDirectory(), `${id}.html`), 'utf8');
    reply
      .type('text/html; charset=utf-8')
      .header('Cache-Control', 'public, max-age=31536000, immutable')
      .header('Content-Security-Policy', 'sandbox allow-scripts allow-forms allow-popups allow-downloads')
      .header('X-Content-Type-Options', 'nosniff');
    return html;
  });

  app.post('/api/artifacts', async (request, reply) => {
    if (!hasIngestAccess(request)) return reply.code(401).send({ error: 'unauthorized' });
    if (!isCreateArtifactBody(request.body)) return reply.code(400).send({ error: 'html and title are required' });

    const { html } = request.body;
    const title = cleanText(request.body.title, 200);
    const tags = (request.body.tags ?? []).map((tag) => cleanText(tag, 50)).filter(Boolean).slice(0, 20);
    const source = cleanText(request.body.source ?? 'manual', 50) || 'manual';
    if (!html || !title) return reply.code(400).send({ error: 'html and title are required' });
    if (Buffer.byteLength(html, 'utf8') > maxHtmlBytes) return reply.code(413).send({ error: 'artifact exceeds 10 MiB limit' });
  ...
  app.delete('/api/artifacts/:id', async (request, reply) => {
    ...
    const deleted = await updateManifest((manifest) => {
      const index = manifest.artifacts.findIndex((artifact) => artifact.id === id);
  ...
  app.patch('/api/artifacts/:id', async (request, reply) => {
    ...
    if (!idPattern.test(id) || !isBucketUpdate(body)) return reply.code(400).send({ error: 'valid id and bucket required' });
