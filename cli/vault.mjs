...
function usage() {
  console.log('Usage: vault push <file.html> [--title "..."] [--tags a,b] [--source name]');
...
async function push(filePath, options) {
  if (!vaultToken) throw new Error('Set VAULT_TOKEN before pushing artifacts.');
  ...
  const result = await request('/api/artifacts', {
    ...
    body: JSON.stringify({ html, title, tags, source: options.source ?? 'manual' }),
  });
...
async function list() {
  ...
  for (const artifact of artifacts) console.log(`${artifact.id}  [${artifact.bucket}]  ${artifact.title}  (${artifact.tags.join(', ')})`);
}

async function remove(id) {
  if (!vaultToken) throw new Error('Set VAULT_TOKEN before deleting artifacts.');
...
async function setBucket(id, bucket) {
  if (!vaultToken) throw new Error('Set VAULT_TOKEN before updating artifacts.');
  if (!id || !['operational', 'understanding'].includes(bucket)) throw new Error('Use an artifact id and operational or understanding.');
  await request(`/api/artifacts/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ bucket }) });
  console.log(`${id}: ${bucket}`);
}

try {
  if (command === 'push') await push(arguments_[0], parseFlags(arguments_.slice(1)));
