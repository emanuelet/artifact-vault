...
beforeEach(async () => {
	...
	process.env.VAULT_READ_TOKEN = "read-token";
...
afterEach(async () => {
	...
	delete process.env.VAULT_READ_TOKEN;
...
describe("artifact API", () => {
	it("requires a token to ingest", async () => {
		const response = await app.inject({
			method: "POST",
			url: "/api/artifacts",
			payload: { html: "<h1>x</h1>", title: "X" },
		});
		expect(response.statusCode).toBe(401);
	});

	it("requires a read token to retrieve the manifest and artifacts", async () => {
		expect(
			(await app.inject({ method: "GET", url: "/api/manifest" })).statusCode,
		).toBe(401);
		expect(
			(
				await app.inject({
		...
		expect(
			(await app.inject({ method: "GET", url: "/artifacts/12345678.html" }))
				.statusCode,
		).toBe(401);
	});

	it("allows the Capacitor origin to send the read token", async () => {
		const response = await app.inject({
			method: "OPTIONS",
			url: "/api/manifest",
			headers: {
	...
	it("allows read-token requests through the passkey gate", async () => {
	...
	it("stores a manifest-backed artifact and serves it sandboxed", async () => {
		...
		expect(manifest.json().artifacts).toMatchObject([
			{ id, bucket: "operational", tags: ["test"] },
		]);
	...
	it("deduplicates HTML and deletes both index and file", async () => {
		const request = {
			...
			url: "/api/artifacts",
			headers: { authorization: "Bearer test-token" },
			payload: { html: "<p>same</p>", title: "One" },
		};
		...
		await expect(
			readFile(join(dataDirectory, "artifacts", `${id}.html`)),
		).rejects.toThrow();
		expect(
			(
				await app.inject({
	...
	it("allows explicit promotion to the understanding bucket", async () => {
		const upload = await app.inject({
			...
			url: "/api/artifacts",
			headers: { authorization: "Bearer test-token" },
			payload: { html: "<p>keep</p>", title: "Keep" },
		});
		const { id } = upload.json() as { id: string };
		const response = await app.inject({
			method: "PATCH",
	...
	it("renders uploaded artifacts in the sandboxed gallery", async () => {
		await app.inject({
			...
			url: "/api/artifacts",
			headers: { authorization: "Bearer test-token" },
			payload: { html: "<p>gallery</p>", title: "Gallery artifact" },
		});
		const response = await app.inject("/");
		expect(response.statusCode).toBe(200);
